...
```text
Cada Agent class define seu provider/model via PHP Attributes ou método `provider()`/`model()`:

#[Provider(Lab::Anthropic)]
#[Model('claude-sonnet-4-6')]
class OrchestratorAgent implements Agent, HasStructuredOutput, HasTools { ... }
→ Planejamento e QA: Claude Sonnet 4-6 (raciocínio mais rigoroso)

#[Provider(Lab::Gemini)]
#[Model('gemini-2.0-flash')]
class BackendSpecialist implements Agent, Conversational, HasTools { ... }
→ Execução de código: Gemini Flash (velocidade + custo)

#[Provider(Lab::Ollama)]
#[Model('qwen2.5:0.5b')]
class ContextCompressor implements Agent { ... }
→ Compressão local: Ollama (sem custo de API, 500MB RAM)
...
### Ferramentas Consolidadas (9 Ferramentas Atômicas — `implements Tool`)
...
| 9 | **MetaTool** | `App\Ai\Tools\MetaTool` | Criar novas ferramentas dinamicamente + logging de impossibilidades |
