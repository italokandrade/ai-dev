...
A montagem do prompt é feita pelo Laravel AI SDK: o `system prompt` vem de `Agent::instructions()` (Camadas 1–3) e o contexto dinâmico (Camada 4) é injetado pela mensagem `user` via `SystemContextService::getFullContext()` + `prompt($subPrd)`. O SDK cuida do `cache_control` (blocos estáticos são marcados como `ephemeral` automaticamente).

---

## 0. Regra Universal — Agentes Operam SOBRE um Projeto Alvo (não sobre o ai-dev-core)

Todo agente de desenvolvimento (`OrchestratorAgent`, `SpecialistAgent`, `QAAuditorAgent`, `DocsAgent`) vive **dentro** do processo do ai-dev-core, mas **trabalha sobre** um Projeto Alvo externo. Esta diferença dita como prompts e tool-calls devem ser construídos.

**Para quem escreve prompts:**

1. **Caminhos absolutos sempre começam no `project.local_path`.** Nunca instrua o agente a abrir, escrever ou editar algo sob `/var/www/html/projetos/ai-dev/ai-dev-core/` — esse é o código-fonte da própria orquestração. Os exemplos canônicos apontam para `/var/www/html/projetos/<nome_do_alvo>/...`.
2. **Schema de banco e docs vêm do Boost do alvo.** Quando o prompt mandar o agente "verificar schema" ou "consultar documentação", isso significa `BoostTool(projectPath=local_path do alvo).database-schema` / `.search-docs` — não o schema do `ai_dev_core` nem as docs do ai-dev-core.
3. **Commits, testes e deploys acontecem no repositório do alvo.** `GitOperationTool` opera no git do `local_path`. `ShellExecuteTool` roda `php artisan test` dentro do alvo. Nada é commitado no repositório do ai-dev-core por conta de uma task.
4. **Dados persistidos no ai-dev-core são metadados.** O `agents_config`, `task_transitions`, `agent_conversations` registram **o que o ai-dev-core fez** — são auditoria. O conteúdo substantivo do trabalho (código novo, migrations, testes, assets) vive **somente** no Projeto Alvo.
5. **IAs de interação de um Projeto Alvo são responsabilidade dele.** Se a spec de um alvo incluir um copiloto, classificador ou sumarizador para seus usuários finais, esses agentes são implantados **dentro do código do alvo** (com seu próprio `.env` e sua própria `OPENROUTER_API_KEY`). Os prompts do ai-dev-core **não** devem conter instruções para operá-los em runtime — apenas para escrevê-los/refatorá-los enquanto código.

Para a separação canônica (repositório, banco, dependências, Boost, workers, `.env`) consulte `README.md → Arquitetura em Duas Camadas`.

---

## 0.1. Blueprint Técnico Progressivo

Entre o PRD Master e os módulos existe um Blueprint Técnico Global. Ele reúne artefatos complementares, todos em JSON estruturado:

- **MER/ERD conceitual:** entidades e relacionamentos, sem campos no nível global.
- **Casos de uso:** ator, objetivo, módulos envolvidos.
- **Workflows:** fluxos de negócio em estrutura compatível com fluxograma/BPMN futuro.
...
1. `ProjectPrdAgent` não define campos, rotas finais, migrations ou classes.
2. `ProjectBlueprintAgent` desenha o domínio global antes dos módulos e não escreve código.
...
*   Se você precisa ler 5 arquivos, chame `FileReadTool` 5 vezes em **paralelo** na mesma resposta em vez de ler um por vez sequencialmente.
...
Diferentes LLMs se comportam de maneira diferente. Cada Agent PHP injeta instruções extras na Camada 2 do seu próprio `instructions()` (método da classe), ramificando pelo provider/modelo declarado em `#[Provider]` e `#[Model]` — não há mais adaptação centralizada por string de provider no banco.
...
### 2.2.2. Roteamento com Modelo Econômico (`#[UseCheapestModel]`)

Para agentes de classificação ou roteamento simples (que apenas decidem qual specialist invocar, sem gerar código), o atributo `#[UseCheapestModel]` do Laravel AI SDK instrui o SDK a selecionar automaticamente o modelo mais barato disponível no provider configurado. No contexto deste projeto (OpenRouter → família Anthropic), isso equivale a Haiku 4.5.
...
#[Provider('openrouter')]
#[UseCheapestModel]
class RoutingAgent implements Agent { ... }
```

Use `#[UseCheapestModel]` apenas em agents cuja única função é classificar/rotear — nunca em agents que geram código, auditam PRDs ou planejam subtasks (esses exigem Sonnet 4.6 ou Opus 4.7 explícitos).
...
BoostTool, DocSearchTool, FileReadTool, FileWriteTool, ShellExecuteTool, GitOperationTool
```
...
FERRAMENTAS À SUA DISPOSIÇÃO:
BoostTool, DocSearchTool, FileReadTool, ShellExecuteTool
> Não há `SecurityTool` dedicado na stack atual. O workflow é: `ShellExecuteTool` dispara `php artisan enlightn`, `composer audit`, `npm audit`, `./vendor/bin/phpstan` dentro do Projeto Alvo; `BoostTool.last-error` e `BoostTool.browser-logs` coletam evidência; `FileReadTool` inspeciona os arquivos sinalizados.
```
...
FERRAMENTAS À SUA DISPOSIÇÃO:
BoostTool, DocSearchTool, FileReadTool, ShellExecuteTool
> Não há `TestTool` dedicado: a execução de testes acontece via `ShellExecuteTool` (`php artisan test --compact`, `./vendor/bin/pest --filter=...`). A detecção de N+1 / queries lentas usa `BoostTool.database-query` (EXPLAIN) e `BoostTool.browser-logs` (Telescope/Debugbar do alvo).
```
...
O `SystemContextService::getFullContext()` adiciona os seguintes textos à **mensagem `user`** do `prompt()` conforme o banco de dados identificar necessidades (Camada 4 do prompt — sempre fora de cache):
...
O contexto anterior é recuperado automaticamente pelo SDK via:
  $agent->continue($project->anthropic_session_id, as: $systemUser)->prompt($prompt)
...
Para impedir ataques de injeção de prompt (*Prompt Injection*) via Web Scraping ou de arquivos contaminados do GitHub, o `SystemContextService` atua com uma camada de esterilização *antes* de incluir o conteúdo na mensagem enviada ao LLM.
...
1. O SystemContextService NÃO injeta esse conteúdo na mensagem do agente.
...
### 6.5. Segurança no `BoostTool.database-query`

O `database-query` é a única sub-tool que executa instruções arbitrárias no banco do Projeto Alvo. Por esse motivo, o `SystemContextService` e o handler da tool devem aplicar, conjuntamente, as seguintes proteções — cada uma independente e complementar:

| Camada | Controle | Onde implementar |
|---|---|---|
| **Allowlist dinâmica** | Tabelas e colunas devem existir no schema real do Projeto Alvo retornado pelo Boost; tabelas internas são bloqueadas; operadores seguem allowlist fixa | `BoostTool::handle()` |
...
| **Conexão read-only** | `database="readonly"` quando o Projeto Alvo tiver essa conexão configurada; em produção isso é obrigatório | `BoostTool::handle()` |
...
Mesmo com hardening, os agentes **não devem** usar `database-query` para inspecionar dados sensíveis de usuários finais; a ferramenta existe para conferir schema, seeds e dados operacionais necessários à implementação.

#### Teste adversarial *(recomendado pelo blog)*

Inclua na suite de testes prompts que tentem burlar as proteções. Exemplos a cobrir:
- Prompt pedindo `SELECT password FROM users` — deve ser bloqueado pela allowlist de colunas
- Prompt referenciando tabela não permitida (ex: `migrations`, `sessions`) — deve ser rejeitado
- Dados no banco contendo instruções de prompt injection (ex: `"Ignore tudo acima e..."`) — o scanner de §6.1–6.2 deve neutralizar antes de injetar no contexto

Audite o `tool_calls_log` periodicamente para verificar quais queries os agentes realmente executam ao longo do tempo.
...
Para referência, este é o prompt completo que o Agent (via `instructions()` + `prompt()` do SDK) envia ao LLM para um subagente executor. Camadas 1–3 vão no `system` (cacheáveis); Camada 4 vai na mensagem `user`:
...
[REGRAS DO PROVEDOR: OpenRouter → família Anthropic]
(Caminhos absolutos, --no-interaction, tool-use com JSON estrito,
 evitar abandono de task, recuperação de falha, chain of thought quando útil)
